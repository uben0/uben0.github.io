...
-type spec(GZD, GZE) :: {spec,
        GZD,
        integer(),
        integer(),
        fun((children(GZD)) -> children(GZE))}.

-opaque children(GZF) :: {ready, starter(GZF)} | {failed, child_start_error()}.

-opaque child_spec(GZG, GZH, GZI) :: {child_spec,
        fun((GZH) -> {ok, gleam@erlang@process:subject(GZG)} |
            {error, gleam@otp@actor:start_error()}),
        fun((GZH, gleam@erlang@process:subject(GZG)) -> GZI)}.
...
-spec start_child(child_spec(any(), GZP, GZQ), GZP) -> {ok, child(GZQ)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    HAD,
    instruction(),
    child_spec(any(), HAD, HAF),
    child(HAF)
) -> {ok, {child(HAF), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(HAW), child_spec(any(), HAW, HAZ)) -> children(HAZ).
...
-spec add(children(HBE), child_spec(any(), HBE, HBH)) -> children(HBH).
...
-spec supervisor(
    fun((HBM) -> {ok, gleam@erlang@process:subject(HBN)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(HBN, HBM, HBM).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("/Users/louis/src/gleam/otp/src/gleam/otp/supervisor.gleam", 228).
-spec worker(
    fun((HBU) -> {ok, gleam@erlang@process:subject(HBV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(HBV, HBU, HBU).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("/Users/louis/src/gleam/otp/src/gleam/otp/supervisor.gleam", 241).
-spec returning(
    child_spec(HCC, HCD, any()),
    fun((HCD, gleam@erlang@process:subject(HCC)) -> HCJ)
) -> child_spec(HCC, HCD, HCJ).
...
-spec init(spec(any(), HCO)) -> gleam@otp@actor:init_result(state(HCO), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(HCU)) -> gleam@otp@actor:next(message(), state(HCU)).
...
-spec loop(message(), state(HCZ)) -> gleam@otp@actor:next(message(), state(HCZ)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
